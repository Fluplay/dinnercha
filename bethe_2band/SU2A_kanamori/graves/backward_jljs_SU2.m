function backward_jljs_SU2 (parfn, varargin)
try
    setenv('RC_STORE',go('rcs')); % use local directory only
    partot = job_func_preamble(parfn,varargin{:});
    for it = (1:numel(partot))
        [ U ,Js, Jl , mu , T , Lambda , Nkeep , nz,  RhoV2init_path, ocont, alphaL, nd, D] = loadvar(partot(it), ...
        {'U','Js','Jl','mu','T','Lambda','Nkeep','nz','RhoV2init_path','ocont','alphaL','nd','D'}, ...
        { [], [] , [], []     , [], [] , [], [],[],	 [], [],[] ,[]      });

        % % result file path
        %resname = go(['glo/DMFT/Bethe_single/Bethe_',par2str('U',U,'mu',mu,'T',T,'Lambda',Lambda,'Nkeep',Nkeep,'nz',nz)]);

        resname = go(['/local/MuNRG/mybin/bethe_2band/SU2A_kanamori/out/backward/Backward_Bethe_jljs_',par2str('U',U,'Jl',Jl,'Js',Js,'T',T,'Lambda',Lambda,'Nkeep',Nkeep,'nz',nz,'alphaL',alphaL,'nd',nd,'D',D)]);
		...
		%%%
		RVs = '/home/dinnercha/local/MuNRG/mybin/bethe_2band/SU2A_kanamori/params/RV2s/'
		disp('++++++++++++++++++++++++++++++++++++++')
		disp("Loaded U is " + [RVs,RhoV2init_path])	
		disp('++++++++++++++++++++++++++++++++++++++')
		RhoV2init = load([RVs,RhoV2init_path]).RhoV2prev;

		iter =5;
		numCorr = 3;
		D = 1;
		get_Bethe_DMFT_jljs_SU2A(U,Js,Jl,mu,iter,numCorr,D,resname,RhoV2init,T,Lambda,Nkeep,nz,alphaL,nd)
		%%%
		result_RhoV2 = load(resname).RhoV2in_arr

		RhoV2prev = result_RhoV2(:,:,end)

		save([RVs,'U',num2str(U),'_backward.mat'])
