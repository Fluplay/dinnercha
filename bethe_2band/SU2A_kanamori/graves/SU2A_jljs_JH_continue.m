function SU2A_jljs_JH_continue (parfn, varargin)
try
    setenv('RC_STORE',go('rcs')); % use local directory only
    partot = job_func_preamble(parfn,varargin{:});
    for it = (1:numel(partot))
        [ U ,Js, Jl , mu , T , Lambda , Nkeep , nz,  RhoV2init ocont, alphaL, nd, D,prev] = loadvar(partot(it), ...
        {'U','Js','Jl','mu','T','Lambda','Nkeep','nz','RhoV2init','ocont','alphaL','nd','D','prev'}, ...
        { [], [] , [], []     , [], [] , [], [],[],	 [], [],[] ,[],[]      });

        % % result file path
        %resname = go(['glo/DMFT/Bethe_single/Bethe_',par2str('U',U,'mu',mu,'T',T,'Lambda',Lambda,'Nkeep',Nkeep,'nz',nz)]);

        resname = go(['/local/MuNRG/mybin/bethe_2band/SU2A_kanamori/out/Bethe_jljs_',par2str('U',U,'Jl',Jl,'Js',Js,'T',T,'Lambda',Lambda,'Nkeep',Nkeep,'nz',nz,'alphaL',alphaL,'nd',nd,'D',D)]);
		...
		numCorr = 3;
		D = 1;
		get_Bethe_DMFT_jljs_SU2A_Zeeman_init_JH_continue(U,Js,Jl,mu,iter,numCorr,D,resname,RhoV2init,T,Lambda,Nkeep,nz,alphaL,nd,prev)
		%%%
