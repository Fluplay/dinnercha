function get_Bethe_DMFT_jljs_SU2A_Zeeman_init_JH_continue(U, ...
						Js, ...
						Jl, ...
						mu, ...
						iter, ...
	...
	nrgdata = path1
    RhoV2_arr = prev.RhoV2_arr;%[];

    RhoV2in_arr = prev.RhoV2in_arr;%RhoV2init;
    G_arr = prev.G_arr;%[];
    A_arr = prev.A_arr;%[];
    SE_arr = prev.SE_arr;%[];
    Delta_arr = prev.Delta_arr;%[];
    nd_arr = prev.nd_arr;%[];
    dd_arr = prev.dd_arr;%[];
	Lz_arr = prev.Lz_arr;%[];
	Lsq_arr = prev.Lsq_arr;%[];
	...
	for i = 1:iter
		...
		[Hepsd,HU,A0,FF,ZF,NF,LF,L2,EF,SF] = get_ham(U,Jl,Js,hl,nd,alphaL,mu);
		...
		[Lz,Lsq] = getEpVal(nrgdataz,LF,L2)

		disp("nn idx : (orbit,spin)")
		disp("Expected nd =>" + nn)
		disp("Expected dd =>" + dd)

		%Flip algorithm (amix or broyden should be accompanied i think)
		if nn(2) > 1.2
			disp("Result is fliped")
			SE = SE(:,end:-1:1);
		end
...
function [Hepsd,HU,A0,FF,ZF,NF,LF,L2,EF,SF] = get_ham (U,Jl,Js,hl,nd,alphaL,mu)
    ...
    HU = (U-3/2*Js)/2*(Ntot-nd*EF)*(Ntot-nd*EF) + Js*(SF2) - Jl*(L2 + alphaL*LF*LF + hl*LF); % benefit on the NF(1)
