function [outputMPO, rank4cell] = getMPOSB(opmatcell, totalid, varargin)
    ...
    for itrow=2:nrow-1
        for itcol=2:ncol-1
            if isempty(opmatcell{itrow, itcol})
                % try
                %     rank4cell{itrow, itcol} = contract(rowid(itrow),2,colid(itcol),1,[1 3 2 4])*near0; % if dimension matchs
                % catch
                    rank4cell{itrow,itcol} = QSpace(); %if dimension does not match
                % end
