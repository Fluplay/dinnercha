function [Mv,E0,normIter,Sv,Hlr,Mmin,normMin] = MPS_MPOproduct_PBO_QSpace (M,Hs,Nsweep,varargin)
    % < Description >
    %
    % [M,E0,normIter,Sv] = DMRG_GS_1site (M,Hs,Nkeep,Nsweep [,'Krylov',nKrylov] [,'tol',tol])
    %
    ...
    % normIter : [N x (2*Nsweep) numeric array] Each element normIter(m,n) means the
    ...
    disptime('MPS-MPO product update');
    disptime(['# of sites = ',sprintf('%i',numel(Hs)),', # of sweeps = ',sprintf('%i',Nsweep),' x 2']);
    
    if ~canonical
        M = canonFormMK(M,N,[]);
    end
    
    % Variational updated MPS
    Mv = M;
    normIter = zeros(N,2*Nsweep);
    ...
    for itS = (1:Nsweep)
        % right -> left
        for itN = (N:-1:1)
            [Mv(itN),normIter(N+1-itN,2*itS-1)] = localContract_PBO_QSpace(Hlr(itN),Hs(itN),Hlr(itN+2),M(itN));
            [Mv(itN),Sv(2*itS-1, itN),Vd] = svdQS(Mv(itN),1,'Nkeep',-1,'stol',0);
            Mv(itN)=QSpace(Mv(itN)); Sv(2*itS-1, itN)=QSpace(Sv(2*itS-1, itN)); Vd=QSpace(Vd); 
    
            % update the next tensor
            if itN > 1
                Mv(itN-1)=contract(Mv(itN-1),2,Vd,2);
                Mv(itN-1)=contract(Mv(itN-1),3,diag(Sv(2*itS-1,itN)),2, [1 3 2]);
            else
                ...
                Mv(itN) = contract(Vd, 1, contract(diag(Sv(2*itS-1, itN)), 1, Mv(itN), 1), 1);
            end
            Hlr(itN+1) = updateLeft(Hlr(itN+2),4, permute(Mv(itN),[2 1 3]), permute(Hs(itN),[1 2 4 3]),4,permute(M(itN),[2 1 3]));
        end
        disptime(['Sweep #',sprintf('%i/%i',[2*itS-1,2*Nsweep]),' (right -> left) : norm = ', ...
            sprintf('%.16g',normIter(N,2*itS-1))]);
        
        % left -> right
        for itN = (1:N)
            [Mv(itN), normIter(itN,2*itS)] = localContract_PBO_QSpace(Hlr(itN),Hs(itN),Hlr(itN+2),M(itN));
            [Mv(itN),Sv(2*itS,itN+1),Vd] = svdQS(Mv(itN),2,'Nkeep',-1,'stol',0);
            Mv(itN) = QSpace(Mv(itN)); Sv(2*itS,itN+1) = QSpace(Sv(2*itS, itN+1)); Vd = QSpace(Vd);
            
            if itN < N
                Mv(itN+1) = contract({diag(Sv(2*itS,itN+1)),'!1',Vd},'!1',Mv(itN+1));
            else
                ...
                Mv(itN) =contract(contract(Mv(itN), 2, diag(Sv(2*itS,itN+1)), 1), 3, Vd, 1, [1 3 2]);
            end
            Hlr(itN+1) = updateLeft(Hlr(itN), 4, Mv(itN), Hs(itN), 4, M(itN));
        end
        disptime(['Sweep #',sprintf('%i/%i',[2*itS,2*Nsweep]),' (left -> right) : norm = ', ...
            sprintf('%.16g',normIter(N,2*itS))]);
        if normIter(N,2*itS) < normMin 
            normMin = normIter(N,2*itS);
            Mmin = Mv;
        end
    end
    
    E0 = normIter(N,2*itS); % take the last value
    toc2(tobj,'-v');
    
    end
    
    function [Amul, normA] = localContract_PBO_QSpace (Hleft,Hcen,Hright,Aold)

    % < Description >
    %
    % [Anew, Enew] = localContract_PBO_QSpace (Hleft,Hcen,Hright,Aold,nKrylov,tol)
    %
    ...
    Amul = contract(Hleft, 2, Aold, 1);
    Amul = contract(Amul, [3 5], Hcen, [3 2]);
    Amul = contract(Amul, [3 5 2], Hright, [2 4 3], [1 3 2]);
    normA = norm(Amul);
