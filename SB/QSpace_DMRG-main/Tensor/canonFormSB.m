function [M,S,dw] = canonFormSB (M,id,Nkeep)
...
% [M,S,dw] = canonFormSB(M,id,Nkeep)

% itags of M and S are 'left', 'right'
% Use alternatively with canonFormMK

% Updated by Subin Kim (12.27.2024)
...
for it = (1:id-1)
    ...
    [M(it),S,Vd, dw(it)] = svdSB(M(it), [1 3], Nkeep,0, itag);
...
for it = (numel(M):-1:id+2)
    ...
    [U, S, M(it), dw(it-1)] = svdSB(M(it), 1, Nkeep,0, itag);
...
if id == 0 % purely right-canonical form
    ...
    [U, S, M(1)] = svdSB(M(1), 1, [],0, itag); % no trucation
    M(1) = legflip(M(1), 1);
elseif id == numel(M) % purely left-canonical form
    ...
    [M(end), S, Vd] = svdSB(M(end), [1 3], [],0, itag); % no trucation
    ...
    [M(id),S,M(id+1), dw(id)] = svdSB(T,[1 2],Nkeep,0, itag);
    M(id) = permute(M(id),[1 3 2]);
end

% bond name near S
S = setitags(S,{'left','right'});
if id > 0
    M(id) = setitags(M(id),2,'left');
end
if id < numel(M)
    M(id+1) = setitags(M(id+1),1,'right');
