function [U, S, Vd, dw] = svdSB(T, inputind, Nkeep, Skeep, varargin)

    % < Description >
    %
    % [U,S,Vd,dw] = svdSB(T, inputind, Nkeep [,itags])
    %
    ...
    %       of singular values.
    %
    % Skeep : [value] Minimal value of singular values to keep. If set empty, it is interpreted as 0, meaning no truncation by the value 
    %< Option >
    ...
    % Rewritten by Subin Kim(Jul. 30, 2024)
    ...
    if isempty(Nkeep) 
        [U, S, Vd, svdinfo] = svdQS(rank2T, 2,'stol',Skeep);
    elseif isint(Nkeep)
        [U, S, Vd, svdinfo] = svdQS(rank2T, 2,'Nkeep',Nkeep,'stol',Skeep);
    else
