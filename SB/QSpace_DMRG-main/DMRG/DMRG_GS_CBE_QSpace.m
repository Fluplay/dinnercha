function [M,E0,Eiter,Sv,dw] = DMRG_GS_CBE_QSpace (M,Hs,Nkeep,Nsweep,delta,alpha,varargin)
    ...
    else % when M is already left canoncal, move center once
        itag = getitags(M(end), 1);
        if ~isempty(itag) && itag(end) == '*'
            itag = itag(1:end-1);
        end
        [U, Sv(1,N), M(end)] = svdMK(M(end), 1, [], itag);
        M(N-1) = contract(M(N-1), 2, U, 1, [1 3 2]);
    end
    Sv(1,N) = diag(Sv(1,N));
        ...
        for itN = (1:N-1)
            ...
            if itN < N-1
                ...
                if SkeepEnd ~= 0 && itS ~=Nsweep
                    M(itN+1) = contract(diag(Sv(2*itS,itN+1)),2,M(itN+1),1);
                    ...
                    [U,Sv(2*itS,itN+1),M(itN+1)] = svdSB(M(itN+1),1,[],[],M(itN+1).info.itags{1});
                    M(itN) = contract(M(itN),2,U,1,[1 3 2]);
                    Sv(2*itS,itN+1) = diag(Sv(2*itS,itN+1));
                end
            end
            Hlr(itN+1) = updateLeft(Hlr(itN), 3, M(itN), Hs(itN), 4, M(itN));
    ...
    % Return Left canonical * Sv(Nsweep*2,N+1) = final state in last process
    M(N) = contract(diag(Sv(2*itS,N)),2,M(N),1);
