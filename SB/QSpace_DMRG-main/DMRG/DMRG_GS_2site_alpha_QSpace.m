function [M,E0,Eiter,Sv, Hlr, Nkeep] = DMRG_GS_2site_alpha_QSpace (M,Hs,Nkeep,Nsweep,alpha,varargin)
    % < Description >
    %
    ...
    tol = 1e-8;
    % ratio of dynamic bond sweep and bond static sweep
    canonical = false;
    raiseFlat = [1 0];
    maxNkeep = Inf;
    makeHlr = true;
    ...
    disptime('Double-site DMRG: ground state search');
    ...
    for itS = (1:Nsweep)
        ...
        for itN = (N-1:-1:1)
            ...
            [Anew,Eiter(N-itN,2*itS-1)] = eigs_2site_GS_QSpace(Hlr(itN),Hs(itN),Hs(itN+1),Hlr(itN+3),Aold,nKrylov,tol);
        ...
        for itN = (1:N-1)
            midtag = M(itN).info.itags{2};
            Aold = contract(M(itN),2,M(itN+1),1,[1,3,2,4]);
            [Anew, Eiter(itN,2*itS)] = eigs_2site_GS_QSpace(Hlr(itN),Hs(itN),Hs(itN+1),Hlr(itN+3),Aold,nKrylov,tol);
