function [M,E0,Eiter,Sv,dw,Mmin,Emin] = DMRG_GS_RSVD_PBO_Arnoldi_QSpace (M,Hs,Nkeep,Nsweep,delta,alpha,varargin)
    % < Description >
    
    % [M,E0,Eiter,Sv] = DMRG_GS_RSVD_QSpace (M,Hs,Nkeep,Nsweep [,'Arnoldi',nArnoldi] [,'tol',tol] [,'Canonical',canonical] [,'svdTol',svdTol] [,'SkeepEnd',SkeepEnd] [,'termTol',termTol])
    ...
    % < Option >
    % 'Arnoldi', .. : [numeric] The maximum dimension of the Arnoldi subspace to
    ...
    %       is diagonalized in each tensor update, with n set by 'Arnoldi',.. .
    %       (Default: 5)
    ...
    if isempty(M(1).info.itags{1})
        Hstart = getIdentity(M(1),1);
    else
        Hstart = getIdentity(M(1),1,strcat(M(1).info.itags{1},'*'));
    end
    if isempty(M(end).info.itags{2})
        Hend = getIdentity(M(end),2); % leg order: bottom (in), top (out),
    else
        Hend = getIdentity(M(end),2,strcat(M(end).info.itags{2},'*')); % leg order: bottom (in), top (out)
    end

    Id = getIdentity(Hs(1),3);

    Hlr(1) = directprod(Hstart,Id); % leg order: bottom (in), top (out)
    Hlr(end) = directprod(Hend,permute(Id,[2 1])); % **NB!** leg order: bottom (in), top (out)
    
    for itN = (1:N-1)
        Hlr(itN+1) = updateLeft(Hlr(itN),4,M(itN),Hs(itN),4,M(itN));
    end
    Mmin = M;
    Emin = Inf;
    iterMax = 3;
    for itS = (1:Nsweep)
        ...
        for itN = (N:-1:2)
            ...
            Hex = updateLeft(Hlr(itN-1),4,Aex,Hs(itN-1),4,Aex);
            Ainit = contract(conj(Aex),[1 3],M(itN-1),[1 3]);
            Ainit = contract(Ainit,2,M(itN),1);
            % tEig = tic2;
            [M(itN),Eiter(N+1-itN,2*itS-1)] = eigs_1site_GS_PBO_QSpace(Hex,Hs(itN),Hlr(itN+2),Ainit,nArnoldi,tol);
            for iter  = 1:iterMax
                [M(itN),Eiter(N+1-itN,2*itS-1)] = eigs_1site_GS_PBO_QSpace(Hex,Hs(itN),Hlr(itN+2),M(itN),nArnoldi,tol);
            end
            % fprintf('Eig time ')
            % toc2(tEig,'-v');
            % SVD
            ...
            % fprintf('D = %i\n',dim(M(itN-1),2));
            Hlr(itN+1) = updateLeft(Hlr(itN+2), 4, permute(M(itN),[2 1 3]), permute(Hs(itN),[1 2 4 3]),4,permute(M(itN),[2 1 3]));
            % fprintf('Else time ')
            % toc2(telse,'-v');
        end
        ...
        for itN = (1:N-1)
            ...
            Hex = updateLeft(Hlr(itN+3),4,permute(Aex,[2 1 3]),permute(Hs(itN+1),[1 2 4 3]),4,permute(Aex,[2 1 3]));
            Ainit = contract(M(itN+1),[2 3],conj(Aex),[2 3]);
            Ainit = contract(M(itN),2,Ainit,1,[1 3 2]);
            [M(itN), Eiter(itN,2*itS)] = eigs_1site_GS_PBO_QSpace(Hlr(itN),Hs(itN),Hex,Ainit,nArnoldi,tol);
            for iter  = 1:iterMax
                [M(itN), Eiter(itN,2*itS)] = eigs_1site_GS_PBO_QSpace(Hlr(itN),Hs(itN),Hex,M(itN),nArnoldi,tol);
            end
        ...
        if Eiter(N-1,2*itS)< Emin && Eiter(N-1,2*itS) > -1
            Emin = Eiter(N-1,2*itS);
            Mmin = M;
        end
...
function [Anew, Enew] = eigs_1site_GS_PBO_QSpace (Hleft,Hcen,Hright,Aold,nArnoldi,tol)

    % < Description >
    %
    % [Anew, Enew] = eigs_1site_GS_PBO_QSpace (Hleft,Hcen,Hright,Aold,nArnoldi,tol)
    %
    ...
    % nArnoldi, tol : Options for the Lacnzos method. See the documentation for
    %       the parent function for details.
    %
    % < Output >
    % Anew : [rank-3 QSpace tensor] The approximate ground state of the effective
    %       Hamiltonian obtained by the Lanczos method, using the Arnoldi
    %       subspace with dimension up to "nArnoldi".
    % Enew : [numeric] Expectation value of the effective Hamiltonian with
    %       respect to "Anew".
    
    Qn = QSpace(nArnoldi,1);
    
    Qn(1) = Aold/norm(Aold);
    
    Hn = zeros(nArnoldi,nArnoldi);
    % Vn = QSpace(nArnoldi,1);

    for itn = (1:nArnoldi)
        % apply transfer matrix
        q = Qn(itn);
        q = contract(Hleft, 2, q, 1);
        q = contract(q, [3 5], Hcen, [3 2]);
        q = contract(q, [3 5 2], Hright, [2 4 3], [1 3 2]);
        for j  = 1:itn
            % double precision
            hn = zeros(1,2);
            for it2 = 1:2
                hn(it2) = getscalarMK(contract(q,[1 2 3],conj(Qn(j)),[1 2 3]));
                q = q - hn(it2)*Qn(j);
            end
            Hn(j,itn) = sum(hn);
        end
    ...
    Anew = V(1,idx)*Qn(1);
    for itK = 2:nArnoldi
        Anew = Anew + V(itK,idx)*Qn(itK);
    end

    AnewH = contract(Hleft, 2, Anew, 1);
    AnewH = contract(AnewH, [3 5], Hcen, [3 2]);
    AnewH = contract(AnewH, [3 5 2], Hright, [2 4 3], [1 3 2]);
    Enew = contract(conj(Anew), [1 2 3], AnewH, [1 2 3]);
    Enew = real(getscalarMK(Enew));
    % residue
    % resTesnor = AnewH - Enew*Anew
    % Anew
...
function Atr = RSVD(HL,HR,Al,Ar,Hl,Hr,Nkeep,delta)
    ...
    TL = contract(TL,[3 5],Hl,[3 2],[1 4 3 5 2]);
    ...
    TR = contract(TR,[3 5],Hr,[4 2],[1 4 3 5 2]);
    ...
    QdM = contract(QdTLd,[2 3 4],TRd,[3 4 5]);
