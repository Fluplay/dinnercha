function [M,E0,Eiter,Sv,Hlr,Mmin,Emin] = DMRG_GS_1site_PBO_QSpace (M,Hs,Nkeep,Nsweep,varargin)
    % < Description >
    %
    ...
    for itS = (1:Nsweep)
        % right -> left
        for itN = (N:-1:1)
            [M(itN),Eiter(N+1-itN,2*itS-1)] = eigs_1site_GS_PBO_QSpace(Hlr(itN),Hs(itN),Hlr(itN+2),M(itN),nKrylov,tol);
        ...
        % left -> right
        for itN = (1:N)
            [M(itN), Eiter(itN,2*itS)] = eigs_1site_GS_PBO_QSpace(Hlr(itN),Hs(itN),Hlr(itN+2),M(itN),nKrylov,tol);
        ...
        if Eiter(N,2*itS) < Emin && Eiter(N,2*itS) > -1
            Emin = Eiter(N,2*itS);
            Mmin = M;
        end
    end
