function [Vex,Dex, Vn, Dn] = eigenTransferMatrixRank3(A,B,nKrylov,tol, varargin)
    ...
    if ~x0input
        error('ERR: Initial guess of the eigenvector is not provided.');
    end
        ...
        for itn = (1:nKrylov)
            ...
            if exclude
                for j = 1:numel(eigenKet)
                    q = q- excludeEig(j)*getscalarMK(contract(Qn(itn),[1 2 3],eigenBra(j),[1 2 3]))*eigenKet(j); % T - \lambda |R><L|
                end
            end
            for j  = 1:itn
                % double precision
