function [Anew, Enew, resNorm] = eigs_HAC_QSpace (Al,h1,Ar,h2,HL,HR,Aold,nKrylov,tol)
    % < Description >
    %
    ...
    for itn = (1:nKrylov)
        ...
        Amul1 = updateLeft([],[],Al,h1,4,Al);
        Amul1 = contract(Amul1,[2 4],As(itn),[1 3],[1 3 2]);
        Amul2 = updateLeft([],[],permute(Ar,[2 1 3]),permute(h2,[3 4 1 2]),4,permute(Ar,[2 1 3]));
        Amul2 = contract(Amul2,[2 4],As(itn),[2 3],[3 1 2]);
        Amul3 = contract(HL,2,As(itn),1);
        Amul4 = contract(As(itn),2,HR,2,[1 3 2]);
        Amul = Amul1+Amul2+Amul3+Amul4;
    ...
    Amul1 = updateLeft([],[],Al,h1,4,Al);
    Amul1 = contract(Amul1,[2 4],Anew,[1 3],[1 3 2]);
    Amul2 = updateLeft([],[],permute(Ar,[2 1 3]),permute(h2,[3 4 1 2]),4,permute(Ar,[2 1 3]));
    Amul2 = contract(Amul2,[2 4],Anew,[2 3],[3 1 2]);
    Amul3 = contract(HL,2,Anew,1);
    Amul4 = contract(Anew,2,HR,2,[1 3 2]);
    Amul = Amul1+Amul2+Amul3+Amul4;

    Enew = real(getscalarMK(contract(Amul,[1 2 3],conj(Anew),[1 2 3])));
